...
class EventTarget {
  ...
  removeEventListener(type, listener) {
    ...
    const index2 = listeners[type].indexOf(listener);
    if (index2 !== -1) {
      listeners[type].splice(index2, 1);
    }
    return this;
  }
  dispatchEvent(event) {
...
class Body extends EventTarget {
  ...
  removeShape(shape) {
    ...
    this.shapeOffsets.splice(index2, 1);
    this.shapeOrientations.splice(index2, 1);
    this.updateMassProperties();
    this.updateBoundingRadius();
    this.aabbNeedsUpdate = true;
    shape.body = null;
...
class Ray {
  ...
  _intersectHeightfield(shape, quat, position, body, reportedShape) {
    ...
    const index2 = intersectHeightfield_index;
    let iMinX;
    let iMinY;
    let iMaxX;
    let iMaxY;
    ...
    shape.getIndexOfPosition(aabb.lowerBound.x, aabb.lowerBound.y, index2, true);
    iMinX = Math.max(iMinX, index2[0]);
    iMinY = Math.max(iMinY, index2[1]);
    shape.getIndexOfPosition(aabb.upperBound.x, aabb.upperBound.y, index2, true);
    iMaxX = Math.min(iMaxX, index2[0] + 1);
    iMaxY = Math.min(iMaxY, index2[1] + 1);
...
class OverlapKeeper {
  ...
  set(i, j) {
    ...
    let index2 = 0;
    while (key > current[index2]) {
      index2++;
    }
    if (key === current[index2]) {
      return;
    }
    for (let j2 = current.length - 1; j2 >= index2; j2--) {
...
class TupleDictionary {
  ...
  delete(i, j) {
    const key = getKey(i, j);
    const index2 = this.data.keys.indexOf(key);
    if (index2 !== -1) {
      this.data.keys.splice(index2, 1);
    }
...
class Physics {
  ...
  async initialiseDebug(scene) {
    if (process.env.NODE_ENV === "development") {
      this.debugger = new (await import("./cannon-es-debugger.c6ab95c8.js")).default(scene, this.world, {});
    }
  }
  add(body) {
  ...
  update() {
    var _a;
    if (process.env.NODE_ENV === "development") {
      (_a = this.debugger) == null ? void 0 : _a.update();
    }
  }
}
